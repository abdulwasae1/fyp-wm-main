...
function MovingHexagons() {
  ...
  // Define our two color values for the gradient
  const color1 = useMemo(() => new THREE.Color("#9081BD"), []); // Purple
  const color2 = useMemo(() => new THREE.Color("#698ABC"), []); // Blue
  ...
  // Create material refs for each hexagon - moved outside of useMemo to follow React Hooks rules
  ...
  useFrame((state, delta) => {
    hexagons.current.forEach((mesh, idx) => {
      ...
      prop.currentT = (state.clock.elapsedTime * 0.5 + offsets[idx]) % (Math.PI * 2);
...
// Helper function to create refs since we can't use useRef in the map callback
function createRef() {
  return { current: null };
}

export default function SidebarModelCanvas() {
  return (
